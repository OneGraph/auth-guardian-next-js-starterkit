export const EXAMPLE_PATH = 'with-auth-guardian'
export const CMS_NAME = 'AuthGuardian'
export const CMS_URL = 'https://www.onegraph.com/auth-guardian/'
export const HOME_OG_IMAGE_URL =
  'https://og-image.now.sh/Next.js%20Blog%20Example%20with%20**DatoCMS**.png?theme=light&md=1&fontSize=75px&images=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Ffront%2Fassets%2Fdesign%2Fnextjs-black-logo.svg&images=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%%2BPC9zdmc%2B&widths=undefined&widths=350&heights=200&heights=auto'

console.log('Build env: ', JSON.stringify(process.env, null, 2))

console.log(
  'next_public env: ',
  JSON.stringify(
    {
      test: 'WORK!',
      NEXT_PUBLIC_ONE_GRAPH_APP_ID:
        process.env.NEXT_PUBLIC_ONE_GRAPH_APP_ID || null,
      NEXT_PUBLIC_GITHUB_ORG: process.env.NEXT_PUBLIC_GITHUB_ORG || null,
      NEXT_PUBLIC_GITHUB_REPO: process.env.NEXT_PUBLIC_GITHUB_REPO || null,
      NEXT_PUBLIC_GIT_PROVIDER: process.env.NEXT_PUBLIC_GIT_PROVIDER || null,
      NEXT_PUBLIC_INSTALL_SETUP_WIZARD_HAS_COMPLETED:
        process.env.NEXT_PUBLIC_INSTALL_SETUP_WIZARD_HAS_COMPLETED || null,
    },
    null,
    2
  )
)

export const ONE_GRAPH_APP_ID =
  process.env.ONE_GRAPH_APP_ID || process.env.NEXT_PUBLIC_ONE_GRAPH_APP_ID
export const ONE_GRAPH_SERVER_SIDE_ACCESS_TOKEN =
  process.env.ONE_GRAPH_SERVER_SIDE_ACCESS_TOKEN

const GITHUB_ORG = process.env.NEXT_PUBLIC_GITHUB_ORG || 'sgrove'
const GITHUB_REPO =
  process.env.NEXT_PUBLIC_GITHUB_REPO || 'throw-away-delete-me-3'
const GIT_PROVIDER = process.env.NEXT_PUBLIC_GIT_PROVIDER || 'github'

export let AUTO_DETECTED_GITHUB_LINK =
  GITHUB_ORG && GITHUB_REPO
    ? `https://github.com/${GITHUB_ORG}/${GITHUB_REPO}`
    : null

export let GIT_CHECKOUT_LINK =
  GITHUB_ORG && GITHUB_REPO
    ? `git@github.com:${GITHUB_ORG}/${GITHUB_REPO}.git`
    : null

// Run https://serve.onegraph.com/short/DJ80ES to find the id of the repository
// you want to use to store feedback
export const FEEDBACK_REPO_ID = 'MDEwOlJlcG9zaXRvcnkxNTUzNzU0NTA=='

// The Next.js AuthGuardian will configure itself on Vercel on the first run, you can delete this afterwards
export const INITIAL_SETUP_WIZARD_HAS_COMPLETED =
  !!(ONE_GRAPH_APP_ID && ONE_GRAPH_SERVER_SIDE_ACCESS_TOKEN) ||
  !!(process.env.NEXT_PUBLIC_INSTALL_SETUP_WIZARD_HAS_COMPLETED === 'true')

export const GIT_PROVIDER_IS_GITHUB = GIT_PROVIDER === 'github'

console.log(
  'Constants:\n',
  `ONE_GRAPH_APP_ID=${ONE_GRAPH_APP_ID}\n`,
  `ONE_GRAPH_SERVER_SIDE_ACCESS_TOKEN=${ONE_GRAPH_SERVER_SIDE_ACCESS_TOKEN}\n`,
  `GITHUB_ORG=${GITHUB_ORG}\n`,
  `GITHUB_REPO=${GITHUB_REPO}\n`,
  `GIT_PROVIDER=${GIT_PROVIDER}\n`,
  `AUTO_DETECTED_GITHUB_LINK=${AUTO_DETECTED_GITHUB_LINK}\n`,
  `GIT_CHECKOUT_LINK=${GIT_CHECKOUT_LINK}\n`,
  `FEEDBACK_REPO_ID=${FEEDBACK_REPO_ID}\n`,
  `INITIAL_SETUP_WIZARD_HAS_COMPLETED=${INITIAL_SETUP_WIZARD_HAS_COMPLETED}\n`,
  `GIT_PROVIDER_IS_GITHUB=${GIT_PROVIDER_IS_GITHUB}\n`
)

export const ONE_GRAPH_SITE_HOST = 'https://www.onegraph.com'
